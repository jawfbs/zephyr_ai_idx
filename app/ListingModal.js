...
import { X, Heart, Share2, EyeOff, Bed, Bath, MapPin, Maximize2, Clock, DollarSign, Phone, Mail, Calendar } from 'lucide-react'
import { formatPrice } from './data'
import NeighborhoodPanel from './NeighborhoodPanel'

export default function ListingModal({ listing, c, t, onClose, isSaved, onToggleSaved, onHide, user, onXP }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9998, backdropFilter: 'blur(6px)', padding: '16px' }}
      onClick={onClose}>
      <div onClick={e => e.stopPropagation()}
        style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: '20px', width: '100%', maxWidth: '680px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: `0 40px 100px rgba(0,0,0,0.6), 0 0 0 1px ${t.accent}20` }}>
        ...
        <div style={{ position: 'relative', height: '240px', flexShrink: 0, overflow: 'hidden' }}>
          ...
          <button onClick={onClose}
            style={{ position: 'absolute', top: '14px', right: '14px', width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(0,0,0,0.6)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', backdropFilter: 'blur(8px)' }}>
            <X size={16} />
          </button>
          {user && (
          ...
          <div style={{ position: 'absolute', bottom: '14px', left: '16px' }}>
            <p style={{ color: '#fff', fontWeight: 900, fontSize: '28px', margin: 0, textShadow: '0 2px 12px rgba(0,0,0,0.6)' }}>{formatPrice(listing.price)}</p>
            <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '13px', margin: 0 }}>{listing.address} · {listing.city}, {listing.state} {listing.zip}</p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', borderBottom: `1px solid ${c.border}`, flexShrink: 0, background: c.surface, overflowX: 'auto' }}>
          {tabs.map(tab_ => (
            <button key={tab_.id} onClick={() => setTab(tab_.id)}
              style={{ padding: '12px 18px', border: 'none', background: 'transparent', cursor: 'pointer', fontSize: '13px', fontWeight: tab === tab_.id ? 700 : 500, color: tab === tab_.id ? t.accent : c.textMuted, borderBottom: `2px solid ${tab === tab_.id ? t.accent : 'transparent'}`, whiteSpace: 'nowrap', transition: 'all 0.2s' }}>
              {tab_.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
          ...
          {tab === 'details' && (
            <div>
              ...
              {listing.description && (
                <div style={{ padding: '14px', borderRadius: '12px', background: c.surfaceAlt, border: `1px solid ${c.border}` }}>
          ...
          {tab === 'contact' && (
            <div>
              ...
              {[
                { icon: Phone, label: 'Call Agent',    sub: '+1 (800) 555-0199', color: '#22c55e' },
