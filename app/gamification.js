...
export function getLevel(xp)     { return [...LEVELS].reverse().find(l=>xp>=l.minXP)||LEVELS[0] }
export function getNextLevel(xp) { return LEVELS.find(l=>l.minXP>xp)||null }
...
export const DEFAULT_STATS = {
  totalXP:0, viewCount:0, savedCount:0, shareCount:0, searchCount:0,
  loginStreak:1, layersUsed:[], filtersUsed:[], neighborhoodsExplored:0,
  walkscore:false, crimeLookup:false, weatherCheck:false,
  achievements:[], xpLog:[], lastLogin:null,
}

export function addXP(stats, action) {
  const def = XP_ACTIONS[action]
  if (!def) return stats
  const newXP = (stats.totalXP||0) + def.xp
  const entry = { action, xp:def.xp, label:def.label, icon:def.icon, ts:Date.now() }
  return { ...stats, totalXP:newXP, xpLog:[entry,...(stats.xpLog||[])].slice(0,50) }
}
