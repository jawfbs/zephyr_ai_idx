    ...
    ;(async () => {
      try {
        ...
        const features = listings
          ...
          .map(listing => {
            const f = new Feature({
              geometry: new Point(fromLonLat([listing.lon, listing.lat])),
            })
            f.setId(listing.id)
        ...
        const center = hasCoords
          ? fromLonLat([hasCoords.lon, hasCoords.lat])
          : fromLonLat([-96.7898, 46.8772]) // Fargo default

        const map = new OLMap({
          target: mapRef.current,
          layers: [tileLayer, vLayer],
          view:   new View({ center, zoom: hasCoords ? 11 : 10 }),
        })
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {popupInfo && (
        <div style={{
          position: 'absolute',
          top:  `${Math.min(popupInfo.pixel[1] - 10, window.innerHeight - 320)}px`,
          left: `${Math.min(popupInfo.pixel[0] + 10, window.innerWidth  - 250)}px`,
          zIndex: 600,
          ...
          <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.7)', margin: '0 0 2px' }}>
            {popupInfo.listing.address}
          </p>
          ...
          <button
            onClick={() => {
              if (onSelectListing) onSelectListing(popupInfo.listing)
              setPopupInfo(null)
            }}
            style={{
