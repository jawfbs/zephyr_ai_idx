...
const SPARK_BASE = {
  live:        'https://api.sparkplatform.com/v1',
  replication: 'https://replication.sparkplatform.com/api/v1',
}

// ── Auth header formats to try in order ──────────────────────────────────────
...
// ── Map a raw Spark listing to ZephyrAI schema ────────────────────────────────
function mapListing(raw, idx) {
  ...
  const price  = parseFloat(s.ListPrice   || s.CurrentPrice || 0)
  const beds   = parseInt(s.BedsTotal     || s.Bedrooms     || 0)
  const baths  = parseFloat(s.BathsTotal  || s.BathroomsTotalDecimal || s.BathroomsFull || 0)
  const sqft   = parseInt(s.BuildingAreaTotal || s.LivingArea || s.AboveGradeFinishedArea || 0)

  const streetNum  = s.StreetNumber    || ''
  const streetDir  = s.StreetDirPrefix || ''
  const streetName = s.StreetName      || ''
  const streetSuf  = s.StreetSuffix    || ''
  const address    = [streetNum, streetDir, streetName, streetSuf]
...
// ── Try one fetch with given auth header ──────────────────────────────────────
async function tryFetch(url, authValue) {
...
export async function GET(request) {
  ...
  if (!apiKey) {
    return NextResponse.json({ success:false, source:'no_credentials', listings:[] })
  }

  const base   = SPARK_BASE[mode] || SPARK_BASE.replication
  const filter = buildFilter(query)

  // Spark field list — request only what we need
  const fields = [
    'ListingId','StandardStatus','MlsStatus','Status',
    'ListPrice','CurrentPrice',
    'StreetNumber','StreetDirPrefix','StreetName','StreetSuffix','UnparsedAddress','StreetAddress',
  ...
  for (const fmt of authHeaders(apiKey, apiSecret)) {
    try {
      console.log('[Spark Listings] Trying:', fmt.name)
      const res = await tryFetch(url, fmt.value)

      console.log('[Spark Listings]', fmt.name, '→ status:', res.status)

      if (res.status === 429) {
        return NextResponse.json({ success:false, source:'rate_limit', message:'Rate limit hit', listings:[] })
      }

      if (!res.ok) continue  // try next auth format

      const data = await res.json()
      const raw  = data?.D?.Results || data?.Results || data?.value || []

      console.log('[Spark Listings] Raw count:', raw.length)
  ...
  return NextResponse.json({
    success: false,
    source:  'auth_failed',
    message: 'All authentication formats failed. Check your API key and Spark account permissions.',
    listings: [],
  })
}
