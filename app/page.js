...
export default function ZephyrPage() {
  ...
  // ── Grant XP ─────────────────────────────────────────────────────────────────
  const grantXP = useCallback((action, overrideStats) => {
    if (!gamificationEnabled) return
    const def = XP_ACTIONS[action]
    if (!def) return

    setUserStats(prev => {
      const current = overrideStats || prev
      const next    = addXP(current, action)

      // check new achievements
      const newOnes = checkAchievements(next, next.achievements || [])
      let finalStats = next
      if (newOnes.length > 0) {
        finalStats = { ...next, achievements: [...(next.achievements || []), ...newOnes.map(a => a.id)] }
        setNewAchievement(newOnes[0])
        // grant achievement XP
        newOnes.forEach(a => {
          finalStats = addXP(finalStats, 'VIEW_LISTING') // re-use for +xp, override below
          finalStats = { ...finalStats, totalXP: finalStats.totalXP - (XP_ACTIONS.VIEW_LISTING?.xp || 0) + a.xp }
        })
      }
    ...
    setToasts(prev => [...prev, { id, xp: def.xp, label: def.label, icon: def.icon }])
  ...
  const handleSaveApiCreds = useCallback((creds) => {
    ...
    fetch(`/api/listings?query=${encodeURIComponent(query)}`)
      .then(r => r.json())
      .then(data => { setListings(data.listings?.length > 0 ? data.listings : DEMO_LISTINGS); setLoading(false) })
  ...
  const cycleMode = () => setColorMode(prev => prev==='light'?'mellow':prev==='mellow'?'dark':'light')
  ...
  const currentLevel = getLevel(userStats.totalXP || 0)
  ...
  return (
    <div style={{ display:'flex',flexDirection:'column',height:'100vh',overflow:'hidden',fontFamily:'"Inter",system-ui,sans-serif',backgroundColor:c.bg,color:c.text,transition:'background 0.3s,color 0.3s' }}>
      ...
      {/* ── Achievement popup ── */}
      {newAchievement && (
        <div style={{ position:'fixed',top:'80px',left:'50%',transform:'translateX(-50%)',zIndex:99999,background:'rgba(15,15,25,0.97)',backdropFilter:'blur(16px)',border:`1px solid ${t.accent}60`,borderRadius:'16px',padding:'16px 24px',boxShadow:`0 24px 60px rgba(0,0,0,0.6),0 0 0 1px ${t.accent}20`,display:'flex',alignItems:'center',gap:'14px',minWidth:'280px' }}>
          ...
          <button onClick={() => setNewAchievement(null)} style={{ background:'none',border:'none',color:'rgba(255,255,255,0.4)',cursor:'pointer',fontSize:'16px',marginLeft:'auto',padding:'4px' }}>✕</button>
        </div>
      )}
      ...
      {gamificationOpen && (
        <GamificationPanel c={c} t={t} stats={userStats} onClose={() => setGamificationOpen(false)} />
      )}
      {selectedListing && (
        <ListingModal
        ...
        <div style={{ display:'flex',alignItems:'center',gap:'8px' }}>
          ...
          {gamificationEnabled && user && (
            <button onClick={() => setGamificationOpen(true)}
              ...
              onMouseLeave={e => { e.currentTarget.style.background=`${t.accent}15`; e.currentTarget.style.borderColor=`${t.accent}50` }}>
              <span style={{ fontSize:'14px' }}>{currentLevel.badge}</span>
          ...
          <div style={{ position:'relative' }} ref={accordionRef}>
            ...
            {accordionOpen && (
              /* ── KEY FIX: isolate dropdown so it never overlaps the map ── */
              <div style={{
                position:'absolute',top:'100%',right:0,marginTop:'8px',
                backgroundColor:c.surface,border:`1px solid ${c.border}`,
                borderRadius:'14px',boxShadow:'0 16px 48px rgba(0,0,0,0.3)',
                padding:'8px',zIndex:300,         /* z-index 300 stays above header (100) and search (90) but below modals (9998) */
                width:'300px',maxHeight:'75vh',overflowY:'auto',
                /* Ensure it does NOT affect layout below it */
                isolation:'isolate',
              }}>
            ...
            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'18px' }}>
              ...
              {gamificationEnabled && user && (
                <button onClick={() => setGamificationOpen(true)}
                  ...
                  <div style={{ textAlign:'left' }}>
                    <p style={{ fontSize:'11px',fontWeight:800,color:t.accent,margin:0 }}>{currentLevel.badge} {currentLevel.title}</p>
