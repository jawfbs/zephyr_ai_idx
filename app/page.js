...
import { useZephyrUser } from './ClerkUserSync'
...
function MoreFiltersModal({ c, t, filters, setFilters, listings, onClose }) {
  ...
  const matchCount = all.filter(l => {
    ...
    if (filters.keywords     && !l.description?.toLowerCase().includes(filters.keywords.toLowerCase())) return false
    return true
  }).length
  ...
  const Label = ({ children }) => (
    <p style={{ fontSize:'11px', fontWeight:700, color:c.textMuted, textTransform:'uppercase', letterSpacing:'1px', margin:'0 0 8px' }}>{children}</p>
  )
  ...
  const inp = (key, placeholder, type='text') => (
    <input type={type} placeholder={placeholder} value={filters[key]||''} onChange={e=>set(key,e.target.value)}
      ...
      <div onClick={e=>e.stopPropagation()}
          ...
          <Row>
            ...
            <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
              {['Any','1','1.5','2','2.5','3','4+'].map(b=>{
                ...
                return <button key={b} onClick={()=>set('minBaths',on?'':val)} style={{ padding:'7px 14px', borderRadius:'8px', border:`1px solid ${on?t.accent:c.border}`, background:on?t.accent:'transparent', color:on?'#fff':c.text, cursor:'pointer', fontSize:'13px', fontWeight:600, transition:'all 0.15s' }}>{b}</button>
              })}
            </div>
          </Row>
          <Row>
          ...
          <Row>
            ...
            <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
              {[['Any',''],['1 day','1'],['7 days','7'],['14 days','14'],['30 days','30'],['60 days','60'],['90+ days','90']].map(([label,val])=>{
                ...
                return <button key={label} onClick={()=>set('maxDom',on?'':val)} style={{ padding:'7px 14px', borderRadius:'8px', border:`1px solid ${on?t.accent:c.border}`, background:on?t.accent:'transparent', color:on?'#fff':c.text, cursor:'pointer', fontSize:'12px', fontWeight:600, transition:'all 0.15s' }}>{label}</button>
              })}
            </div>
          </Row>
          <Row>
            ...
            <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
              {['Any','Active','Pending','Coming Soon','Active Under Contract'].map(s=>{
                const on=s==='Any'?!filters.status:filters.status===s
                return <button key={s} onClick={()=>set('status',s==='Any'?'':on?'':s)} style={{ padding:'7px 14px', borderRadius:'8px', border:`1px solid ${on?t.accent:c.border}`, background:on?t.accent:'transparent', color:on?'#fff':c.text, cursor:'pointer', fontSize:'12px', fontWeight:600, transition:'all 0.15s' }}>{s}</button>
              })}
            </div>
          </Row>
          <Row>
          ...
          <Row>
            ...
            <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
              {[['Any',''],['0.1 ac','0.1'],['0.25 ac','0.25'],['0.5 ac','0.5'],['1 ac','1'],['2+ ac','2']].map(([label,val])=>{
                ...
                return <button key={label} onClick={()=>set('minLot',on?'':val)} style={{ padding:'7px 14px', borderRadius:'8px', border:`1px solid ${on?t.accent:c.border}`, background:on?t.accent:'transparent', color:on?'#fff':c.text, cursor:'pointer', fontSize:'12px', fontWeight:600, transition:'all 0.15s' }}>{label}</button>
              })}
            </div>
          </Row>
          <Row last>
            <Label>Keywords in Description</Label>
            <input type="text" placeholder='"corner lot", "updated kitchen", "cul-de-sac"' value={filters.keywords||''} onChange={e=>set('keywords',e.target.value)}
        ...
        <div style={{ padding:'16px 20px', borderTop:`1px solid ${c.border}`, display:'flex', gap:'10px', flexShrink:0 }}>
          <button onClick={()=>setFilters({})}
            style={{ flex:1, padding:'11px', borderRadius:'10px', border:`1px solid ${c.border}`, background:'transparent', color:c.textMuted, cursor:'pointer', fontSize:'13px', fontWeight:700 }}
...
export default function ZephyrPage() {
  ...
  // ── Clerk auth ──────────────────────────────────────────────────────────────
  const { user: clerkUser, isSignedIn, isLoaded } = useZephyrUser()
      ...
      .then(data => {
        if (data.success && data.listings?.length > 0) {
          ...
          if (data.source === 'auth_error') {
            console.warn('[ZephyrAI] Spark auth error:', data.message)
          }
        }
        setLoading(false)
      })
  ...
  const handleSearch = (e) => {
    ...
    setLoading(true)
    if (apiCreds?.apiKey) {
      const params = new URLSearchParams({ query, apiKey:apiCreds.apiKey, apiSecret:apiCreds.apiSecret||'', mode:apiCreds.mode||'replication', limit:'50' })
      fetch(`/api/listings?${params.toString()}`)
        .then(r=>r.json())
        .then(data => {
          if (data.success && data.listings?.length>0) setListings(data.listings)
          else {
            const q=query.toLowerCase()
            setListings(listings.filter(l=>!query||l.city.toLowerCase().includes(q)||l.address.toLowerCase().includes(q)||l.zip.includes(q)) || listings)
          }
          setLoading(false)
        })
    ...
    setTimeout(() => {
      const q=query.toLowerCase()
      const filtered=DEMO_LISTINGS.filter(l=>!query||l.city.toLowerCase().includes(q)||l.address.toLowerCase().includes(q)||l.zip.includes(q)||l.state.toLowerCase().includes(q))
  ...
  const displayed = listings
    ...
    .filter(l => {
      ...
      if (mf.minBaths    && l.baths < parseFloat(mf.minBaths))                        return false
      if (mf.minSqft     && l.sqft  < parseInt(mf.minSqft))                           return false
      if (mf.maxSqft     && l.sqft  > parseInt(mf.maxSqft))                           return false
      if (mf.yearBuiltMin && (l.yearBuilt||0) < parseInt(mf.yearBuiltMin))            return false
      if (mf.yearBuiltMax && (l.yearBuilt||0) > parseInt(mf.yearBuiltMax))            return false
      if (mf.maxDom      && l.daysOnMarket > parseInt(mf.maxDom))                     return false
      if (mf.status      && l.status !== mf.status)                                   return false
      if (mf.keywords    && !l.description?.toLowerCase().includes(mf.keywords.toLowerCase())) return false
      return true
    })
    .sort((a,b) => {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', overflow:'hidden', fontFamily:'"Inter",system-ui,sans-serif', backgroundColor:c.bg, color:c.text, transition:'background 0.3s,color 0.3s', position:'relative' }}>
      ...
      {/* Bold Tech overlays */}
      {activeLayoutId==='bold_tech' && <div style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:0, backgroundImage:'linear-gradient(rgba(0,245,255,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(0,245,255,0.04) 1px,transparent 1px)', backgroundSize:'40px 40px', opacity:0.6 }} />}
      {activeLayoutId==='bold_tech' && <div style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:0, backgroundImage:'repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,245,255,0.015) 2px,rgba(0,245,255,0.015) 4px)' }} />}
      ...
      {/* Modals */}
      {tutorialOpen && <Tutorial c={c} t={t} onClose={()=>setTutorialOpen(false)} onSignIn={()=>setSignInOpen(true)} />}
      ...
      {moreFiltersOpen  && <MoreFiltersModal c={c} t={t} filters={moreFilters} setFilters={setMoreFilters} listings={listings} onClose={()=>setMoreFiltersOpen(false)} />}
      {selectedListing  && (
        <ListingModal listing={selectedListing} c={c} t={t}
          onClose={()=>setSelectedListing(null)}
          isSaved={saved.includes(selectedListing.id)}
          onToggleSaved={toggleSaved} onHide={handleHide}
          user={user} onXP={grantXP}
          allListings={displayed} featureSettings={featureSettings}
        />
      )}
      {featureListing && (
        <FeaturePanel listing={featureListing} allListings={displayed} t={t} c={c}
      ...
      <header style={{
        ...
        justifyContent:'space-between', flexShrink:0, zIndex:100, position:'relative',
      }}>
            ...
            {contactOpen && (
              <div style={{ position:'absolute', top:'100%', right:0, marginTop:'8px', backgroundColor:c.surface, border:`1px solid ${c.border}`, borderRadius:'14px', boxShadow:'0 16px 48px rgba(0,0,0,0.3)', padding:'16px', zIndex:200, width:'300px' }}>
                ...
                {[
                  ...
                  <button key={label}
                    style={{ width:'100%', display:'flex', alignItems:'center', gap:'12px', padding:'10px', borderRadius:'10px', border:'none', background:'transparent', cursor:'pointer', marginBottom:'4px' }}
          ...
          <div style={{ position:'relative' }} ref={accordionRef}>
            <button onClick={()=>{ setAccordionOpen(!accordionOpen); setContactOpen(false) }}
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'3.5px' }}>
                {[16,12,16].map((w,i)=><span key={i} style={{ display:'block', width:`${w}px`, height:'1.5px', background:'currentColor', borderRadius:'2px' }} />)}
              </div>
              <User size={15} />
            </button>
                    ...
                    <div style={{ padding:'12px', borderRadius:'10px', background:c.surfaceAlt, marginBottom:'8px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'8px' }}>
                        <div style={{ width:'40px', height:'40px', borderRadius:'50%', background:t.gradient, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, boxShadow:`0 0 12px ${t.accentGlow}` }}>
                          {user.imageUrl
                            ? <img src={user.imageUrl} alt="" style={{ width:'40px', height:'40px', borderRadius:'50%', objectFit:'cover' }} />
      ...
      <div style={{ backgroundColor:c.surface, borderBottom:`1px solid ${c.border}`, padding:'14px 20px 12px', flexShrink:0, boxShadow:c.searchShadow, zIndex:90, position:'relative' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'8px', flexWrap:'wrap' }}>
              ...
              <div style={{ ...dropdown, width:'210px', padding:'8px' }}>
                {['All Types',...propertyTypes].map(type=>(
                  <button key={type} onClick={()=>{ setFilterType(type==='All Types'?'':type); setTypeOpen(false); grantXP('USE_FILTER') }}
                    style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 12px', borderRadius:'8px', border:'none', background:'transparent', cursor:'pointer', fontSize:'13px', color:c.text }}
                    onMouseEnter={e=>e.currentTarget.style.background=c.surfaceAlt}
                    onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
          ...
          {/* More */}
          <button style={pill(Object.values(moreFilters).some(v=>v&&v!==''))} onClick={()=>setMoreFiltersOpen(true)}>
            <SlidersHorizontal size={13} /> More
            {Object.values(moreFilters).filter(v=>v&&v!=='').length>0 && (
              <span style={{ background:t.accent, color:'#fff', borderRadius:'10px', fontSize:'9px', fontWeight:800, padding:'1px 6px', marginLeft:'2px' }}>
                {Object.values(moreFilters).filter(v=>v&&v!=='').length}
              </span>
            )}
          </button>

          {/* Active moreFilter pills */}
          {moreFilters.minBaths    && <span style={{ display:'flex',alignItems:'center',gap:'4px',padding:'5px 10px',borderRadius:'16px',background:`$${t.accent}15`,border:`1px solid $${t.accent}40`,fontSize:'11px',fontWeight:600,color:t.accent,cursor:'pointer',whiteSpace:'nowrap' }} onClick={()=>setMoreFiltersOpen(true)}>🛁 {moreFilters.minBaths}+ ba <span onClick={e=>{e.stopPropagation();setMoreFilters(p=>({...p,minBaths:''}))}} style={{ marginLeft:'2px',fontWeight:900 }}>×</span></span>}
          {moreFilters.minSqft     && <span style={{ display:'flex',alignItems:'center',gap:'4px',padding:'5px 10px',borderRadius:'16px',background:`$${t.accent}15`,border:`1px solid $${t.accent}40`,fontSize:'11px',fontWeight:600,color:t.accent,cursor:'pointer',whiteSpace:'nowrap' }} onClick={()=>setMoreFiltersOpen(true)}>📐 {parseInt(moreFilters.minSqft).toLocaleString()}+ ft² <span onClick={e=>{e.stopPropagation();setMoreFilters(p=>({...p,minSqft:''}))}} style={{ marginLeft:'2px',fontWeight:900 }}>×</span></span>}
          {moreFilters.maxDom      && <span style={{ display:'flex',alignItems:'center',gap:'4px',padding:'5px 10px',borderRadius:'16px',background:`$${t.accent}15`,border:`1px solid $${t.accent}40`,fontSize:'11px',fontWeight:600,color:t.accent,cursor:'pointer',whiteSpace:'nowrap' }} onClick={()=>setMoreFiltersOpen(true)}>⏰ ≤{moreFilters.maxDom}d <span onClick={e=>{e.stopPropagation();setMoreFilters(p=>({...p,maxDom:''}))}} style={{ marginLeft:'2px',fontWeight:900 }}>×</span></span>}
          {moreFilters.status      && <span style={{ display:'flex',alignItems:'center',gap:'4px',padding:'5px 10px',borderRadius:'16px',background:`$${t.accent}15`,border:`1px solid $${t.accent}40`,fontSize:'11px',fontWeight:600,color:t.accent,cursor:'pointer',whiteSpace:'nowrap' }} onClick={()=>setMoreFiltersOpen(true)}>📋 {moreFilters.status} <span onClick={e=>{e.stopPropagation();setMoreFilters(p=>({...p,status:''}))}} style={{ marginLeft:'2px',fontWeight:900 }}>×</span></span>}
          {moreFilters.yearBuiltMin && <span style={{ display:'flex',alignItems:'center',gap:'4px',padding:'5px 10px',borderRadius:'16px',background:`$${t.accent}15`,border:`1px solid $${t.accent}40`,fontSize:'11px',fontWeight:600,color:t.accent,cursor:'pointer',whiteSpace:'nowrap' }} onClick={()=>setMoreFiltersOpen(true)}>🏗️ {moreFilters.yearBuiltMin}+ <span onClick={e=>{e.stopPropagation();setMoreFilters(p=>({...p,yearBuiltMin:''}))}} style={{ marginLeft:'2px',fontWeight:900 }}>×</span></span>}
          {moreFilters.hasPool     && <span style={{ display:'flex',alignItems:'center',gap:'4px',padding:'5px 10px',borderRadius:'16px',background:`$${t.accent}15`,border:`1px solid $${t.accent}40`,fontSize:'11px',fontWeight:600,color:t.accent,cursor:'pointer',whiteSpace:'nowrap' }} onClick={()=>setMoreFiltersOpen(true)}>🏊 Pool <span onClick={e=>{e.stopPropagation();setMoreFilters(p=>({...p,hasPool:''}))}} style={{ marginLeft:'2px',fontWeight:900 }}>×</span></span>}
          {moreFilters.hasGarage   && <span style={{ display:'flex',alignItems:'center',gap:'4px',padding:'5px 10px',borderRadius:'16px',background:`$${t.accent}15`,border:`1px solid $${t.accent}40`,fontSize:'11px',fontWeight:600,color:t.accent,cursor:'pointer',whiteSpace:'nowrap' }} onClick={()=>setMoreFiltersOpen(true)}>🚗 Garage <span onClick={e=>{e.stopPropagation();setMoreFilters(p=>({...p,hasGarage:''}))}} style={{ marginLeft:'2px',fontWeight:900 }}>×</span></span>}
          {moreFilters.keywords    && <span style={{ display:'flex',alignItems:'center',gap:'4px',padding:'5px 10px',borderRadius:'16px',background:`$${t.accent}15`,border:`1px solid $${t.accent}40`,fontSize:'11px',fontWeight:600,color:t.accent,cursor:'pointer',whiteSpace:'nowrap',maxWidth:'140px',overflow:'hidden',textOverflow:'ellipsis' }} onClick={()=>setMoreFiltersOpen(true)}>🔤 "{moreFilters.keywords}" <span onClick={e=>{e.stopPropagation();setMoreFilters(p=>({...p,keywords:''}))}} style={{ marginLeft:'2px',fontWeight:900,flexShrink:0 }}>×</span></span>}
                    ...
                    <div style={{ position:'relative', flexShrink:0, width:viewMode==='list'?'220px':'100%' }}
                      ...
                      {user && (
                        <div style={{ position:'absolute', top:'10px', right:'10px', display:'flex', gap:'5px', zIndex:15 }}>
                          <button onClick={e=>{e.stopPropagation();toggleSaved(listing.id)}} style={{ width:'28px', height:'28px', borderRadius:'50%', background:isSaved_?t.accent:'rgba(0,0,0,0.5)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.2s' }}>
                            <Heart size={13} style={{ fill:isSaved_?'#fff':'none', color:'#fff' }} />
                          </button>
                          <button onClick={e=>{e.stopPropagation();grantXP('SHARE_LISTING');setUserStats(ps=>{const ns={...ps,shareCount:(ps.shareCount||0)+1};saveStats(ns);return ns})}} style={{ width:'28px', height:'28px', borderRadius:'50%', background:'rgba(0,0,0,0.5)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
