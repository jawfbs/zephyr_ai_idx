...
{tutorialOpen && (
        ...
        <div style={{ display:'flex',alignItems:'center',gap:'8px' }}>
          {gamificationEnabled && user && (
            <button onClick={() => setGamificationOpen(true)}
              style={{ display:'flex',alignItems:'center',gap:'6px',padding:'6px 12px',borderRadius:'10px',border:`1px solid ${t.accent}50`,background:`${t.accent}15`,cursor:'pointer' }}
          ...
          <button onClick={cycleMode}
            style={{ width:'38px',height:'38px',borderRadius:'10px',border:`1px solid ${c.border}`,background:c.surfaceAlt,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:t.accent }}
          ...
          <div style={{ position:'relative' }} ref={contactRef}>
            <button onClick={() => { setContactOpen(!contactOpen); setAccordionOpen(false) }}
              style={{ width:'38px',height:'38px',borderRadius:'10px',border:`1px solid ${contactOpen?t.accent:c.border}`,background:contactOpen?`${t.accent}15`:c.surfaceAlt,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:contactOpen?t.accent:c.textMuted }}>
            ...
            {contactOpen && (
              <div style={{ position:'absolute',top:'100%',right:0,marginTop:'8px',backgroundColor:c.surface,border:`1px solid ${c.border}`,borderRadius:'14px',boxShadow:'0 16px 48px rgba(0,0,0,0.3)',padding:'16px',zIndex:200,width:'300px' }}>
                <div style={{ display:'flex',alignItems:'center',gap:'10px',marginBottom:'16px' }}>
                  <div style={{ width:'40px',height:'40px',borderRadius:'10px',background:t.gradient,display:'flex',alignItems:'center',justifyContent:'center' }}><MessageSquare size={18} color="#fff" /></div>
                  <div>
                ...
                {[
                  ...
                  <button key={label}
                    ...
                    <div style={{ width:'34px',height:'34px',borderRadius:'8px',background:`${color}20`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}><Icon size={16} style={{ color }} /></div>
          ...
          <div style={{ position:'relative' }} ref={accordionRef}>
            <button onClick={() => { setAccordionOpen(!accordionOpen); setContactOpen(false) }}
              style={{ display:'flex',alignItems:'center',gap:'8px',padding:'7px 14px',borderRadius:'10px',border:`1px solid ${accordionOpen?t.accent:c.border}`,background:accordionOpen?`${t.accent}15`:c.surfaceAlt,cursor:'pointer',color:accordionOpen?t.accent:c.text,fontSize:'13px',fontWeight:600 }}>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap' }}>
          <div style={{ position:'relative' }}>
            <button style={pill(filterPriceMin||filterPriceMax)} onClick={() => { closeAllFilters(); setPriceOpen(!priceOpen) }}>
              ...
              <ChevronDown size={12} style={{ transform:priceOpen?'rotate(180deg)':'none' }} />
            </button>
            {priceOpen && (
          ...
          <div style={{ position:'relative' }}>
            <button style={pill(filterBeds)} onClick={() => { closeAllFilters(); setBedsOpen(!bedsOpen) }}>
              ...
              <ChevronDown size={12} style={{ transform:bedsOpen?'rotate(180deg)':'none' }} />
            </button>
            {bedsOpen && (
          ...
          <div style={{ position:'relative' }}>
            <button style={pill(filterType)} onClick={() => { closeAllFilters(); setTypeOpen(!typeOpen) }}>
              ...
              <ChevronDown size={12} style={{ transform:typeOpen?'rotate(180deg)':'none' }} />
            </button>
            {typeOpen && (
          ...
          <div style={{ position:'relative' }}>
            <button style={pill(false)} onClick={() => { closeAllFilters(); setSortOpen(!sortOpen) }}>
              ...
              <ChevronDown size={12} style={{ transform:sortOpen?'rotate(180deg)':'none' }} />
            </button>
            {sortOpen && (
              <div style={{ ...dropdown,width:'210px',padding:'8px' }}>
                {sortOptions.map(opt=>(
            ...
            <div style={{ display:'flex',border:`1px solid ${c.border}`,borderRadius:'10px',overflow:'hidden',background:c.surfaceAlt }}>
              {[{mode:'grid',icon:LayoutGrid},{mode:'list',icon:List},{mode:'split',icon:Map}].map(({mode,icon:Icon})=>(
                <button key={mode} onClick={() => { setViewMode(mode); if(mode==='split') grantXP('VIEW_MAP') }}
                  style={{ padding:'7px 11px',border:'none',borderLeft:mode!=='grid'?`1px solid ${c.border}`:'none',background:viewMode===mode?t.accent:'transparent',color:viewMode===mode?'#fff':c.textMuted,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center' }}>
                  <Icon size={15} />
                </button>
              ))}
            </div>
      ...
      <div style={{ display:'flex',flex:1,overflow:'hidden' }}>
        <div style={{ width:viewMode==='split'?'50%':'100%',overflowY:'auto',padding:'20px',backgroundColor:c.bg }}>
          ...
          {!loading && (
            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'18px' }}>
              ...
              {gamificationEnabled && user && (
                <button onClick={() => setGamificationOpen(true)}
                  style={{ display:'flex',alignItems:'center',gap:'8px',padding:'8px 14px',borderRadius:'12px',border:`1px solid ${t.accent}40`,background:`${t.accent}10`,cursor:'pointer' }}
                  onMouseEnter={e=>e.currentTarget.style.background=`${t.accent}20`}
              ...
              {displayed.map(listing => {
                ...
                return (
                  <div key={listing.id}
                    ...
                    style={{ background:c.cardBg,borderRadius:'16px',overflow:'hidden',border:`1px solid ${isHovered?t.accent+'60':c.border}`,cursor:'pointer',transition:'all 0.25s',boxShadow:isHovered?`0 0 0 1px ${t.accent}40,0 8px 32px rgba(0,0,0,0.15)`:'0 1px 4px rgba(0,0,0,0.06)',transform:isHovered?'translateY(-3px)':'none',display:viewMode==='list'?'flex':'block' }}>
                    ...
                    <div style={{ position:'relative',flexShrink:0,width:viewMode==='list'?'220px':'100%' }}
                      ...
                      <div style={{ position:'absolute',top:'10px',left:'10px',display:'flex',gap:'5px',zIndex:15,pointerEvents:'none' }}>
                        ...
                        {isNew&&<span style={{ background:t.accent,color:'#fff',fontSize:'10px',fontWeight:700,padding:'3px 8px',borderRadius:'20px' }}>✦ NEW</span>}
                      </div>
                      {user && (
                        <div style={{ position:'absolute',top:'10px',right:'10px',display:'flex',gap:'5px',zIndex:15 }}>
                          <button onClick={e=>{e.stopPropagation();toggleSaved(listing.id)}} style={{ width:'28px',height:'28px',borderRadius:'50%',background:isSaved_?t.accent:'rgba(0,0,0,0.5)',border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center' }}>
                          ...
                          <button onClick={e=>{e.stopPropagation();handleHide(listing.id)}} style={{ width:'28px',height:'28px',borderRadius:'50%',background:'rgba(0,0,0,0.5)',border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center' }}
                      ...
                      <div style={{ position:'absolute',bottom:'28px',left:'12px',zIndex:15,pointerEvents:'none' }}>
                        <span style={{ color:'#fff',fontWeight:900,fontSize:'19px',textShadow:'0 2px 8px rgba(0,0,0,0.6)' }}>{formatPrice(listing.price)}</span>
                      </div>
                    </div>

                    <div style={{ padding:'12px',flex:1 }} onClick={() => { setSelectedListing(listing); grantXP('VIEW_LISTING') }}>
