...
export default function AccordionSection({
  ...
  const [notifPrefs,  setNotifPrefs]  = useState({ email: true, browser: false, priceDrops: true, newListings: true, savedUpdates: true })
  ...
  const handleSaveCreds = () => {
    ...
    setTimeout(() => {
      setSaving(false)
      setSavedState(true)
      if (onSaveApiCreds) onSaveApiCreds({ apiKey, apiSecret, mode: apiMode })
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 0', borderBottom: `1px solid ${c.border}` }}>
      ...
      <button
        ...
        style={{
          ...
          background: value ? t.accent : c.border, cursor: 'pointer',
          position: 'relative', transition: 'background 0.25s', flexShrink: 0, marginLeft: '12px',
        }}>
        <span style={{
      ...
      {open && (
        <div style={{ padding: '8px 12px 12px' }}>
          ...
          {section.isNotifications && (
            <div>
              <Toggle value={notifPrefs.email}       onChange={v => setNotifPrefs(p=>({...p,email:v}))}       label="Email Alerts"     sub="Listing updates via email" />
              <Toggle value={notifPrefs.browser}     onChange={v => setNotifPrefs(p=>({...p,browser:v}))}     label="Browser Push"     sub="Real-time push notifications" />
              <Toggle value={notifPrefs.priceDrops}  onChange={v => setNotifPrefs(p=>({...p,priceDrops:v}))}  label="Price Drops"      sub="Alert when prices fall" />
              <Toggle value={notifPrefs.newListings} onChange={v => setNotifPrefs(p=>({...p,newListings:v}))} label="New Listings"     sub="Matching your saved searches" />
              <Toggle value={notifPrefs.savedUpdates}onChange={v => setNotifPrefs(p=>({...p,savedUpdates:v}))}label="Saved Home Updates"sub="Status changes on saved homes" />
            </div>
          )}
              ...
              {gamificationEnabled && (
                <>
                  <div style={{ marginTop: '12px', padding: '12px', borderRadius: '10px', background: c.surfaceAlt, border: `1px solid ${c.border}` }}>
                    ...
                    <button
                      onClick={() => { onOpenGamification && onOpenGamification() }}
          ...
          {/* ── Integrations (SparkAPI) ── */}
          {section.isIntegrations && (
            <div>
              <p style={{ fontSize: '12px', color: c.textMuted, marginBottom: '12px' }}>Connect your SparkAPI credentials to pull live MLS data.</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', marginBottom: '10px' }}>
                {['replication','live'].map(mode => (
                  <button key={mode} onClick={() => setApiMode(mode)}
                    style={{ padding: '8px', borderRadius: '8px', border: `1px solid ${apiMode === mode ? t.accent : c.border}`, background: apiMode === mode ? `${t.accent}20` : 'transparent', color: apiMode === mode ? t.accent : c.textMuted, cursor: 'pointer', fontSize: '12px', fontWeight: 600, transition: 'all 0.2s', textTransform: 'capitalize' }}>
                    {mode}
                  </button>
                ))}
              </div>
              {[['API Key', apiKey, setApiKey, 'sk-...'], ['API Secret', apiSecret, setApiSecret, '••••••••']].map(([label, val, setter, ph]) => (
                <div key={label} style={{ marginBottom: '8px' }}>
                  ...
                  <input type={label === 'API Secret' ? 'password' : 'text'} value={val} onChange={e => setter(e.target.value)} placeholder={ph}
                    style={{ width: '100%', padding: '9px 10px', border: `1px solid ${c.border}`, borderRadius: '8px', fontSize: '12px', background: c.inputBg, color: c.text, outline: 'none', boxSizing: 'border-box' }}
                    onFocus={e => e.target.style.borderColor = t.accent}
                    onBlur={e => e.target.style.borderColor = c.border} />
                </div>
              ))}
              <button onClick={handleSaveCreds} disabled={saving || !apiKey || !apiSecret}
                style={{ width: '100%', padding: '10px', borderRadius: '8px', border: 'none', background: saved ? '#16a34a' : (!apiKey || !apiSecret) ? c.border : t.gradient, color: '#fff', cursor: !apiKey || !apiSecret ? 'not-allowed' : 'pointer', fontSize: '13px', fontWeight: 700, transition: 'all 0.3s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                {saving ? '⏳ Connecting…' : saved ? '✅ Connected!' : '🔌 Connect SparkAPI'}
              </button>
            </div>
          )}
