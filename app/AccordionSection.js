...
export default function AccordionSection({ section, c, t, colorMode, setColorMode, themeCategory, setThemeCategory, activeTheme, setActiveTheme, THEMES, onOpenAccount, onSaveApiCreds, gamificationEnabled, setGamificationEnabled, onOpenGamification, userStats }) {
    ...
    <div style={{ borderRadius:'10px', overflow:'hidden', marginBottom:'4px', border:`1px solid ${open?t.accent+'40':'transparent'}`, transition:'border 0.2s' }}>
      ...
      {open&&(
        <div style={{ padding:'8px 12px 12px' }}>
          ...
          {section.isAccount&&(
            <button onClick={onOpenAccount} style={{ width:'100%', padding:'10px', borderRadius:'10px', border:`1px solid ${c.border}`, background:c.surfaceAlt, cursor:'pointer', fontSize:'13px', fontWeight:600, color:c.text, transition:'all 0.2s', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}
            ...
            <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'6px', marginBottom:'14px' }}>
                {[['light','☀️ Light'],['mellow','🌆 Mellow'],['dark','🌑 Dark']].map(([id,label])=>(
                  <button key={id} onClick={()=>setColorMode(id)} style={{ padding:'8px 4px', borderRadius:'8px', border:`1px solid ${colorMode===id?t.accent:c.border}`, background:colorMode===id?`${t.accent}20`:'transparent', color:colorMode===id?t.accent:c.text, cursor:'pointer', fontSize:'11px', fontWeight:600, transition:'all 0.2s' }}>{label}</button>
                ))}
              </div>
            ...
            <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'6px', marginBottom:'10px' }}>
                {['replication','live'].map(mode=>(
                  <button key={mode} onClick={()=>setApiMode(mode)} style={{ padding:'8px', borderRadius:'8px', border:`1px solid ${apiMode===mode?t.accent:c.border}`, background:apiMode===mode?`${t.accent}20`:'transparent', color:apiMode===mode?t.accent:c.textMuted, cursor:'pointer', fontSize:'12px', fontWeight:600, transition:'all 0.2s', textTransform:'capitalize' }}>{mode}</button>
                ))}
              </div>
